...
export default function DashPosts() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const res = await fetch(`https://media-one-rust.vercel.app/api/post/getposts?userId=${currentUser._id}`);
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(
        `https://media-one-rust.vercel.app/api/post/getposts?userId=${currentUser._id}&startIndex=${startIndex}`
      );
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDeletePost = async () => {
    ...
    try {
      const res = await fetch(
        `https://media-one-rust.vercel.app/api/post/deletepost/${postIdToDelete}/${currentUser._id}`,
        {
          method: 'DELETE',
        }
      );
