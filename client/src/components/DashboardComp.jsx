...
export default function DashboardComp() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await fetch(`${import.meta.env.SERVER_DOMAIN}/api/user/getusers?limit=5`);
    ...
    const fetchPosts = async () => {
      try {
        const res = await fetch(`${import.meta.env.SERVER_DOMAIN}/api/post/getposts?limit=5`);
        const data = await res.json();
        if (res.ok) {
          setPosts(data.posts);
    ...
    const fetchComments = async () => {
      try {
        const res = await fetch(`${import.meta.env.SERVER_DOMAIN}/api/comment/getcomments?limit=5`);
        const data = await res.json();
        if (res.ok) {
